...
Interesting to note that almost the same performance was obtained with 12 less features than the original model.  Therefore, the remaining models will be trained with these same regressors.
...
Finally, Random Forest obtained a better accuracy than this model.  Just in case, again, the same modelling was carried on with all possible regressors.  Almost the same results were obtaine.  Therefore, p-value analysis was indeed a useful method for selecting final regressors in this problem.
...
As said previously, due to non converging GAM combining model, the final model to be tested with Validation Data, was Random Forest.
