...
features2 <- reformulate(termlabels = namelist[-grep("gyro", namelist, value = FALSE)], response = 'classe')
modfitmlog2 <- train(features2, method = "multinom", data = training[, -c(1:6)])
...
```{r, include=FALSE}
modfitrf <- train(classe ~ ., method = "rf", data = training[,-c(1:6)])
