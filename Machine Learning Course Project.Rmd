...
Finally, after removing the only row with the 3 remaining NAs, the training dataset with which to start working went down to the following dimensions (detailed structure of the obtained dataset can be seen in Appendix 1):

```{r}
training <- training[-which(rowSums(is.na(training)) != 0),]
dim(training)
```
